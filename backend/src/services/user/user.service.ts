...
import { ValidateResult } from 'src/shared/models';
import { UserWorkTypeRepository } from 'src/repositories/user-work-type.repository';

@Injectable()
export class UserService {
  constructor(
    private readonly userRepository: UserRepository,
    private readonly userRecoverRepository: UserRecoverRepository,
    private readonly userWorkTypeRepository: UserWorkTypeRepository
  ) {}
  ...
  async add(model: CreateUser): Promise<void> {
    const user: User = {
      email: model.email,
      name: model.name,
      type: model.type,
    ...
    await this.userRepository.db.transaction(async (trx) => {
      const userId = await this.userRepository.add(user, trx);
      user.id = userId;

      if (model.workTypeId) {
        await this.userWorkTypeRepository.add(
          {
            workTypeId: model.workTypeId,
            userId,
            description: 'Diga-nos sobre seus serviços prestados!',
          },
          trx
        );
      }
  ...
  private async validateUserEntity(entity: User): Promise<ValidateResult> {
    let result = true;
    const errors: string[] = [];
