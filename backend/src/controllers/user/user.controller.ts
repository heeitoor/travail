...
import { ActivateUser, ChangePassword, LoginUser } from 'src/services/user/models';
...
export class UserController {
  ...
  @Post('/customer')
  async postCustomer(@Body() model: User): Promise<void> {
    model.type = UserType.CUSTOMER;
    return await this.userService.add(model);
  }

  @Post('/professional')
  async postProfessional(@Body() model: User): Promise<void> {
    model.type = UserType.PROFESSIONAL;
    return await this.userService.add(model);
  }

  @Post('/company')
  async postCompany(@Body() model: User): Promise<void> {
    model.type = UserType.COMPANY;
    return await this.userService.add(model);
  }
